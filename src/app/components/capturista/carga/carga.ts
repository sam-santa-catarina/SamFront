...
interface FilaPreview {
  ...
  programa: string;
  concepto: string;
  // Solo aplica para 'registrados' (pendientes)
  estatus?: string;
    ...
    return new Promise((resolve, reject) => {
      ...
      reader.onload = () => {
        try {
          ...
          const iEstatus = indiceDe('ESTATUS');
          ...
          for (let i = HEADER_ROW_INDEX + 1; i < filas.length; i++) {
            ...
            if (tab === 'registrados') {
              filaPreview.estatus = celda(iEstatus);
            }
            ...
            const fechaRaw = fila[iFecha];
            if (typeof fechaRaw === 'number') {
              filaPreview.fecha = excelSerialToDateString(fechaRaw);
            } else {
